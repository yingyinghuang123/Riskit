...
async function main() {
  const outDir = path.join(__dirname, 'output/蝎子');
  fs.mkdirSync(outDir, { recursive: true });

  for (const card of CARDS) {
    const canvas = await renderCard(card);
    const fileName = `${String(card.id).padStart(2, '0')}_蝎子_${card.kind}_T${card.threat}_V${card.treasure}.png`;
    const buf = canvas.toBuffer('image/png');
    fs.writeFileSync(path.join(outDir, fileName), buf);
    console.log(`✓ ${fileName}`);
  }
  console.log(`\n完成！共 ${CARDS.length} 张，输出到 output/蝎子/`);
}
